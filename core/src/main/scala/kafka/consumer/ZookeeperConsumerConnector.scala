  ...
  private def ensureOffsetManagerConnected() {
    if (config.offsetsStorage == "kafka") {
      if (offsetsChannel == null || !offsetsChannel.isConnected)
        offsetsChannel = ClientUtils.channelToOffsetManager(config.groupId, config.channelType, zkClient, config.offsetsChannelSocketTimeoutMs, config.offsetsChannelBackoffMs)
