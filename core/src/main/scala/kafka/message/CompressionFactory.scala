...
import kafka.utils.MessageLengthChecksum
  ...
  def apply(compressionCodec: CompressionCodec, stream: OutputStream): OutputStream = {
    compressionCodec match {
      ...
      case LZ4CompressionCodec =>
        ...
        new LZ4BlockOutputStream(stream, 1 << 16, LZ4Factory.fastestInstance().fastCompressor(), new MessageLengthChecksum, false)
      case _ =>
  ...
  def apply(compressionCodec: CompressionCodec, stream: InputStream): InputStream = {
    compressionCodec match {
      ...
      case LZ4CompressionCodec =>
        ...
        new LZ4BlockInputStream(stream, LZ4Factory.fastestInstance().fastDecompressor(), new MessageLengthChecksum)
      case _ =>
        throw new kafka.common.UnknownCodecException("Unknown Codec: " + compressionCodec)
    }
  }
