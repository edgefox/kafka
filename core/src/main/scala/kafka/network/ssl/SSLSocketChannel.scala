...
import kafka.network.KafkaChannel
...
object SSLSocketChannel {
  ...
  def makeSecureClientConnection(sch: SocketChannel, host: String, port: Int) = {
    val engine = SSLAuth.sslContext.createSSLEngine(host, port)
  ...
  def makeSecureServerConnection(socketChannel: SocketChannel,
    ...
    val engine = socketChannel.socket.getRemoteSocketAddress match {
      case ise: InetSocketAddress =>
        SSLAuth.sslContext.createSSLEngine(ise.getHostName, ise.getPort)
      case _ =>
        SSLAuth.sslContext.createSSLEngine()
    }
    engine.setEnabledProtocols(Array("SSLv3"))
...
class SSLSocketChannel(val socketChannel: SocketChannel, val sslEngine: SSLEngine)
  extends KafkaChannel(socketChannel) with Logging {
        ...
        outer.synchronized {
                             ...
                             handshakeStatus match {
                               case HandshakeStatus.NEED_WRAP =>
                                 debug("sslTasker setting up to write for %s".format(socketChannel.socket.getRemoteSocketAddress))
                               ...
                               case HandshakeStatus.NEED_UNWRAP =>
                                 if (peerNetData.position > 0) {
                                   ...
                                   debug("sslTasker setting up to read for %s".format(socketChannel.socket.getRemoteSocketAddress))
                                   selectionKey.interestOps(SelectionKey.OP_READ)
                                 }
                               case HandshakeStatus.NEED_TASK =>
                                 val runnable = sslEngine.getDelegatedTask
                                 if (runnable != null) {
                                   debug("sslTasker running next task for %s".format(socketChannel.socket.getRemoteSocketAddress))
  ...
  override def connect(remote: SocketAddress): Boolean = {
    ...
    val ret = socketChannel.connect(remote)
    if (blocking) {
      while (!finishConnect()) {
        try {
          Thread.sleep(10)
      ...
      blockingKey = socketChannel.register(blockingSelector, SelectionKey.OP_READ)
  ...
  override def read(dst: ByteBuffer): Int = {
    this.synchronized {
                        ...
                        } else if (socketChannel.socket.isInputShutdown) {
  ...
  override def read(destination: Array[ByteBuffer], offset: Int, length: Int): Long = {
    var n = 0
    var i = offset
    def localReadLoop() {
      while (i < length) {
  ...
  override def write(source: ByteBuffer): Int = {
    this.synchronized {
                        if (myNetData.hasRemaining) {
                          writeRaw(myNetData)
                          return 0
                        } else if (socketChannel.socket.isOutputShutdown) {
                          throw new ClosedChannelException
                        } else if (initialized != 0) {
  ...
  override def write(sources: Array[ByteBuffer], offset: Int, length: Int): Long = {
    var n = 0
    var i = offset
    def localWriteLoop {
      while (i < length) {
  ...
  override def toString = "SSLSocketChannel[" + socketChannel.toString + "]"
  ...
  override protected def implConfigureBlocking(block: Boolean) {
      ...
      while (true) {
        handshakeStatus match {
          case HandshakeStatus.NOT_HANDSHAKING =>
            info("begin ssl handshake for %s/%s".format(socketChannel.socket.getRemoteSocketAddress,
                                                        socketChannel.socket.getLocalSocketAddress))
            sslEngine.beginHandshake()
            handshakeStatus = sslEngine.getHandshakeStatus
          case HandshakeStatus.NEED_UNWRAP =>
            debug("need unwrap in ssl handshake for %s/%s".format(socketChannel.socket.getRemoteSocketAddress,
                                                                  socketChannel.socket.getLocalSocketAddress))
            if (readIfReadyAndNeeded(true) && handshakeStatus != HandshakeStatus.FINISHED) {
              debug("select to read more for %s/%s".format(socketChannel.socket.getRemoteSocketAddress,
                                                           socketChannel.socket.getLocalSocketAddress))
              return SelectionKey.OP_READ
            }
          case HandshakeStatus.NEED_WRAP =>
            debug("need wrap in ssl handshake for %s/%s".format(socketChannel.socket.getRemoteSocketAddress,
                                                                socketChannel.socket.getLocalSocketAddress))
            if (myNetData.remaining == 0) {
              wrap(emptyBuffer)
            }
            if (writeIfReadyAndNeeded(true)) {
              debug("select to write more for %s/%s".format(socketChannel.socket.getRemoteSocketAddress,
                                                            socketChannel.socket.getLocalSocketAddress))
          ...
          case HandshakeStatus.FINISHED =>
            info("finished ssl handshake for %s/%s".format(socketChannel.socket.getRemoteSocketAddress,
                                                           socketChannel.socket.getLocalSocketAddress))
            return 0
          case null =>
  ...
  private[this] def readRaw(): Long = {
    ...
    this.synchronized {
                        blockIfNeeded()
                        try {
                          val n = socketChannel.read(peerNetData)
                          if (n < 0) {
  ...
  private[this] def writeRaw(out: ByteBuffer): Long = {
    ...
    try {
      if (out.hasRemaining) {
        socketChannel.write(if (SSLSocketChannel.simulateSlowNetwork) writeTwo(out) else out)
      } else 0
    } catch {
      case x: IOException =>
  ...
  private[this] def runTasks(ops: Int = SelectionKey.OP_READ): HandshakeStatus = {
    val reInitialize = initialized match {
      case 0 =>
        initialized = ops
        info("runTasks running renegotiation for %s/%s".format(socketChannel.socket.getRemoteSocketAddress,
                                                               socketChannel.socket.getLocalSocketAddress))
        true
      case _ => false
    }
    ...
    if (!blocking && selectionKey != null) {
      debug("runTasks asynchronously in ssl handshake for %s/%s".format(socketChannel.socket.getRemoteSocketAddress,
                                                                        socketChannel.socket.getLocalSocketAddress))
      if (runnable != null) {
        executor.execute(new SSLTasker(runnable))
      }
      ...
      debug("runTasks synchronously in ssl handshake for %s/%s".format(socketChannel.socket.getRemoteSocketAddress,
                                                                       socketChannel.socket.getLocalSocketAddress))
      while (runnable != null) {
        runnable.run()
  ...
  override def forKey(key: SelectionKey, selectOption: Int = -1): KafkaChannel = {
    if (selectOption >= 0 && !finished()) {
      var done = false
      try {
        val next = handshake(key.interestOps(), key)
        if (next == 0) {
          // when handshake is complete and we are doing a read so ahead with the read
          // otherwise go back to read mode
          if (selectOption == SelectionKey.OP_READ) {
            done = true
          } else {
      ...
      if (done) this else null
    } else this
  }

  override def isExtraReadRequired = isReadable
}
